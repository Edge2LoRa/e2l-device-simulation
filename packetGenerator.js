...
function packetGenerator(DevAddr, AppSKey, NwkSKey, FPort, FCnt) {
  ...
  // encode the payload
  const array = [
    parseFloat(date.substring(date.length - 4, date.length - 2)),
    parseFloat(date.substring(date.length - 2, date.length))
  ];
  ...
  */
  let headerPKTFWD =
      new Uint8Array([ 2, 45, 141, 0, 184, 39, 235, 255, 254, 230, 15, 44 ]);
  ...
  mergedArray.set(headerPKTFWD,
                  0); // Copy headerPKTFWD to mergedArray at the beginning.
  mergedArray.set(
      json,
      headerPKTFWD.length); // Copy json to mergedArray after headerPKTFWD.
  return mergedArray;
}
