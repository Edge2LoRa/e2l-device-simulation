...
const sendPacketToAllGWs = (packet, frameLosses, socket_arrays) => {
  for (const index in socket_arrays) {
    const socket = socket_arrays[index];
    let frameLoss;
    if (frameLosses instanceof Array)
      frameLoss = frameLosses[index % frameLosses.length];
    else frameLoss = frameLosses;
    if (socket) {
...
function simulateDevice(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const index in packetsToSend) {
      ...
      let frameLosses = [];
      switch (frameLoss_value) {
        case -2: // MOBILITY
          // let gw_number = socket_arrays.length;
          let divisor = 3;
          let chunk_len = nPackets / divisor;
          let chunk_index = Math.floor(FCnt / chunk_len);
          ...
          switch (chunk_index) {
            case 0:
              ...
              break;
            case 1:
              let modifier = ((FCnt % chunk_len) * 100) / chunk_len;
              if (modifier > 100) modifier = 100;
              frameLosses = [modifier, 100 - modifier];
              break;
            case 2:
              frameLosses = [100, 0];
              break;
            default:
              frameLosses = calculateLoss();
          }
          break;
        case -1: // COMPUTED
          frameLosses = calculateLoss();
          break;
        default:
          if (frameLoss_value < 0 || frameLoss_value > 100) {
            return reject(
              `Invalid frameLoss value: ${frameLoss_value}. It must be between 0 and 100.`
            );
          }
          frameLosses = frameLoss_value;
      }

      sendPacketToAllGWs(packet, frameLosses, socket_arrays);
      console.log(
